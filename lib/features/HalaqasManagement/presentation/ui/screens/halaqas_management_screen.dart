  ...
  void _showAddHalaqasDialog(BuildContext context) {
    showModalBottomSheet(
      ...
      builder: (_) => MultiBlocProvider(
        providers: [
          // Reuse the existing HalaqaBloc from the parent screen.
          BlocProvider.value(value: context.read<HalaqaBloc>()),
...
class _AddHalaqaDialogState extends State<AddHalaqaDialog> {
  ...
  void _addForm() {
    setState(() {
      final lastFormValid =
          forms.last.formKey.currentState?.validate() ?? false;
      // Also require a teacher to be selected on the current last form,
      // same as _submitForms does, before allowing a new form to be added.
      final lastTeacherSelected =
          forms.last.stateKey.currentState?.selectedTeacher != null;

      if (lastFormValid && lastTeacherSelected) {
        forms.add(HalaqaForm(key: UniqueKey()));
      } else if (lastFormValid && !lastTeacherSelected) {
  ...
  void _submitForms(BuildContext context) {
    ...
    for (final form in forms) {
      ...
      final halaqaState = form.stateKey.currentState;
      ...
      // Guard: a teacher must be selected before submitting.
      final teacher = halaqaState?.selectedTeacher;
      if (teacher == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
      ...
      final newHalaqa = HalaqaDetailEntity(
        ...
        sumOfStudents: 0,
        maxOfStudents: int.tryParse(form.eneregyController.text.trim()) ?? 0,
        // Use the real teacher ID from the selected entity.
        teacherId: int.tryParse(teacher.id) ?? 0,
      );
  ...
  Widget build(BuildContext context) {
    ...
    // NOTE: No BlocProvider.value here — HalaqaBloc is already provided by
    // the MultiBlocProvider in _showAddHalaqasDialog (it reuses the parent
    // screen's existing bloc instance). Re-providing it here via
    // sl<HalaqaBloc>() would create a disconnected instance and break the
    // submit -> refresh flow between this dialog and the list screen.
    return BlocConsumer<HalaqaBloc, HalaqaState>(
