...
import '../../domain/usecases/check_teacher_username_usecase.dart';
...
import '../../domain/usecases/suggest_teacher_username_usecase.dart';
...
class TeacherBloc extends Bloc<TeacherEvent, TeacherState> {
  ...
  final SuggestTeacherUsernameUseCase _suggestUsername;
  final CheckTeacherUsernameUseCase _checkUsername;
  ...
  TeacherBloc({
    ...
    required SuggestTeacherUsernameUseCase suggestUsernameUC,
    required CheckTeacherUsernameUseCase checkUsernameUC,
       ...
       _suggestUsername = suggestUsernameUC,
       _checkUsername = checkUsernameUC,
    ...
    on<TeacherUsernameRequested>(
      _onSuggestUsername,
      transformer: restartable(),
    );
    on<TeacherUsernameCheckRequested>(
      _onCheckUsername,
      transformer: restartable(),
    );
  }
  ...
  Future<void> _onLoadMore(
    ...
    final result = await _fetchMoreTeachers(nextPage);
  ...
  Future<void> _onDelete(
    ...
    result.fold(
      ...
      (_) => emit(
        state.copyWith(submissionStatus: TeacherSubmissionStatus.success),
      ),
    );
  }
  ...
  Future<void> _onCheckUsername(
    TeacherUsernameCheckRequested event,
    Emitter<TeacherState> emit,
  ) async {
    if (event.name.trim().isEmpty) {
      emit(
        state.copyWith(
          usernameCheckStatus: TeacherUsernameCheckStatus.initial,
          usernameCheck: false,
        ),
      );
      return;
    ...
    emit(
      state.copyWith(usernameCheckStatus: TeacherUsernameCheckStatus.loading),
    );

    final result = await _checkUsername(event.name);

    result.fold(
      (failure) => emit(
        state.copyWith(usernameCheckStatus: TeacherUsernameCheckStatus.failure),
      ),
      (check) => emit(
        state.copyWith(
          usernameCheckStatus: TeacherUsernameCheckStatus.loaded,
          usernameCheck: check,
        ),
      ),
    );
  ...
  Future<void> _onSuggestUsername(
    TeacherUsernameRequested event,
    Emitter<TeacherState> emit,
  ) async {
    if (event.name.trim().isEmpty) {
      emit(
        state.copyWith(
          usernameSuggestionStatus: TeacherUsernameSuggestionStatus.initial,
          usernameSuggestion: '',
        ),
      );
      return;
    ...
    emit(
      state.copyWith(
        usernameSuggestionStatus: TeacherUsernameSuggestionStatus.loading,
      ),
    );

    final result = await _suggestUsername(event.name);

    result.fold(
      (failure) => emit(
        state.copyWith(
          usernameSuggestionStatus: TeacherUsernameSuggestionStatus.failure,
        ),
      ),
      (suggestion) => emit(
        state.copyWith(
          usernameSuggestionStatus: TeacherUsernameSuggestionStatus.loaded,
