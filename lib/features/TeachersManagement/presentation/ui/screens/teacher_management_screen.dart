...
import 'package:shafeea/features/TeachersManagement/presentation/ui/screens/add_teachers_screen.dart';
    ...
    for (var form in forms) {
      if (form.formKey.currentState?.validate() ?? false) {
        ...
        if (form.formKey.currentState?.validate() ?? false) {
          ...
          // Include the username only when the user actually provided one
          // (i.e. the form was shown online and the field is non-empty).
          // When null the server will auto-assign a username.
          final rawUsername = form.usernameController.text.trim();
          ...
          final newTeacher = TeacherDetailEntity(
            ...
            phoneZone: int.tryParse(form.phoneZoneController.text) ?? 0,
            whatsAppPhone: form.whatsAppPhoneController.text,
            whatsAppZone: int.tryParse(form.whatsAppZoneController.text) ?? 0,
            qualification: form.qualificationController.text,
            experienceYears:
                int.tryParse(form.experienceYearsController.text) ?? 0,
