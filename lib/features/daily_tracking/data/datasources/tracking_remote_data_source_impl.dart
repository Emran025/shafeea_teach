...
import 'package:shafeea/features/daily_tracking/data/models/tracking_sync_payload_model.dart';
import 'package:shafeea/features/daily_tracking/data/models/tracking_sync_response_model.dart';
...
final class TrackingRemoteDataSourceImpl implements TrackingRemoteDataSource {
  ...
  TrackingRemoteDataSourceImpl({required ApiConsumer apiConsumer})
    : _apiConsumer = apiConsumer;

  @override
  Future<TrackingSyncResponseModel> pushTrackingUpdates(
    TrackingSyncPayloadModel payload,
  ) async {
    try {
      ...
      if (responseJson is! Map<String, dynamic>) {
        throw const ServerException(
          message:
              'Invalid response format from tracking sync endpoint: Expected a map.',
        );
      }
      ...
      throw ServerException(
        message:
            'An unexpected error occurred during tracking push: ${e.toString()}',
      );
    }
  }
}
